...
def profile(records):
    ...
    for record in records:
        for attempt in record.get("attempts", []):
            ...
            try:
                ...
                sizes = {}
            created, completed = raw.get("created_at"), raw.get("completed_at")
            provider_ms = ((completed - created) * 1000 if isinstance(created, (int, float))
                           and isinstance(completed, (int, float)) and completed >= created else None)
            requests.append({"id": record["id"], "attempt": attempt["number"],
                ...
                "provider_lifecycle_ms": provider_ms, "service_tier": raw.get("service_tier"),
                "output_field_bytes": attempt.get("output_field_bytes", sizes),
                "schema_valid": attempt.get("schema_valid"),
                **usage_metrics(raw)})
    names = ["attempt_wall_ms", "http_wall_ms", "provider_lifecycle_ms", "schema_validation_ms", "policy_latency_ms", "response_json_bytes",
    ...
    distributions["turn_policy_ms"] = distribution([r.get("policy_latency_ms") for r in records if r.get("proposal")])
    ...
    return {"turns": len(records), "requests": len(requests), "distributions": distributions,
            ...
            "observed_service_tiers": sorted({r["service_tier"] for r in requests if r["service_tier"]}),

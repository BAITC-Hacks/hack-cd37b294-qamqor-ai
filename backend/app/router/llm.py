...
class LLMRouter:
    def __init__(self, dataset: Dataset, settings: Settings, client: httpx.AsyncClient):
        ...
        self.stable_message = {"role": "system", "content": [{"type": "input_text",
            "text": system_prompt(self.catalog), "prompt_cache_breakpoint": {"mode": "explicit"}}]}

    def request_payload(self, messages, effort="low", prewarm=False):
        options = {"mode": "explicit", "ttl": "30m"}
        if prewarm:
            options["prewarm"] = True
        return {"model": self.settings.model, "reasoning": {"effort": effort},
                "input": messages, "store": False, "max_output_tokens": 6000,
                "prompt_cache_options": options,
                "text": {"verbosity": "low", "format": {"type": "json_schema", "name": "router_proposal", "strict": True, "schema": self.schema}}}

    async def prewarm(self):
        """Prepare invariant context only. A prewarm is never a routing prediction."""
        self.settings.require_api_key()
        payload = self.request_payload([deepcopy(self.stable_message)], prewarm=True)
        tick = time.perf_counter()
        response = await self.client.post(f"{self.settings.base_url}/responses", json=payload,
            headers={"Authorization": f"Bearer {self.settings.api_key}"}, timeout=self.settings.timeout_seconds)
        attempt = {"operation": "prewarm", "request_payload": payload, "request_sha256": payload_hash(payload),
                   "http_wall_ms": (time.perf_counter() - tick) * 1000,
                   "response_json_bytes": len(response.content), "http_status": response.status_code,
                   "request_id": response.headers.get("x-request-id"), "passed": False}
        if not response.is_error:
            body = response.json()
            attempt.update(raw_response=body, response_id=body.get("id"), **usage_metrics(body))
            attempt["passed"] = body.get("status") == "completed" and not body.get("output") and attempt["output_tokens"] == 0
        return attempt
    ...
    async def route(self, request: RouterInput,
        ...
        latest = state.pop("current_user_turn")
        state["current_user_turn"] = latest
        messages = [deepcopy(self.stable_message),
                    {"role": "user", "content": json.dumps(state, ensure_ascii=False)}]
        for index in range(2):
            effort = "low" if index == 0 else "medium"
            payload = self.request_payload(messages, effort)
